#ifndef WARP_BASIC_FUNCTIONS_CUH 
#define WARP_BASIC_FUNCTIONS_CUH 
    ...
    bool rotate_flag)
{
    ...
    // parallel reduction
    sum1 = warp_reduce_sum<tile_sz>(g, sum1);
    sum2 = warp_reduce_sum<tile_sz>(g, sum2);
...
__device__ __forceinline__ void rosenbrock_function(
    ...
    bool rotate_flag)
{
    scale_shift_and_rotate<tile_sz>(g, x, y, z, dim, shift_data, rotate_data, 0.02048f, shift_flag, rotate_flag);
    // shift to origin
    ...
    partial_solution = warp_reduce_sum<tile_sz>(g, partial_solution);
    if (g.thread_rank() == 0)
...
__device__ __forceinline__ void rastrigin_function(
    ...
    bool rotate_flag)
{
    scale_shift_and_rotate<tile_sz>(g, x, y, z, dim, shift_data, rotate_data, 0.0512f, shift_flag, rotate_flag);
    float s = 0.0f;
    for (int i = g.thread_rank(); i < dim; i += g.size()) // calculation
...
__device__ __forceinline__ void schwefel_function(
    ...
    bool rotate_flag)
{
    scale_shift_and_rotate<tile_sz>(g, x, y, z, dim, shift_data, rotate_data, 10.0f, shift_flag, rotate_flag);
    float s = 0.0f, tmp;
    ...
    s = warp_reduce_sum<tile_sz>(g, s); // parallel reductin
    if (g.thread_rank() == 0)
...
__device__ __forceinline__ void hgbat_function(
    ...
    bool rotate_flag)
{
    scale_shift_and_rotate<tile_sz>(g, x, y, z, dim, shift_data, rotate_data, 0.05f, shift_flag, rotate_flag);
    ...
    r2 = warp_reduce_sum<tile_sz>(g, r2);
    s = warp_reduce_sum<tile_sz>(g, s);
    if (g.thread_rank() == 0)
    ...
    bool rotate_flag)
{
    ...
    s = warp_reduce_product<tile_sz>(g, s); // parallel reduction
    if (g.thread_rank() == 0)
    {
        temp2 = 10.0f / dim / dim; 
    ...
    bool rotate_flag)
{
    ...
    s = warp_reduce_sum<tile_sz>(g, s); // parallel reduction
    r = warp_reduce_sum<tile_sz>(g, r); // parallel reduction
    if (g.thread_rank() == 0)
    ...
    bool rotate_flag)
{
    ...
    sum1 = warp_reduce_sum<tile_sz>(g, sum1); // parallel reduction
    sum2 = warp_reduce_sum<tile_sz>(g, sum2); // parallel reduction
    if (g.thread_rank() == 0)
    {
...
__device__ __forceinline__ void griewank_function(
    ...
    bool rotate_flag)
{
    scale_shift_and_rotate<tile_sz>(g, x, y, z, dim, shift_data, rotate_data, 6.0f, shift_flag, rotate_flag);
    float s = 0.0f;
    float p = 1.0f; 
    for (int i = g.thread_rank(); i < dim; i += g.size())
    {
    ...
    s = warp_reduce_sum<tile_sz>(g, s); // parallel reduction
    p = warp_reduce_product<tile_sz>(g, p); // parallel reduction
    if (g.thread_rank() == 0)
    {
