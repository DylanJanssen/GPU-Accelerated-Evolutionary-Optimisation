...
#include "../numerical_benchmark/cpu/multithreaded_benchmarks.hpp"
...
int main(int argc, char **argv)
{
    ...
    MultithreadedBenchmark benchmark(function_number, dim, popsize);
    ...
    for (int runs = 1; runs <= total_runs; runs++)
    {
        ...
        benchmark.multithreaded_evaluate(de.get_population_ptr(), de.get_fitness_ptr(), popsize);
        ...
        while (evals < max_evaluations && !solution_found)
        {
            ...
            benchmark.multithreaded_evaluate(de.get_offspring_ptr(), de.get_offspring_fitness_ptr(), popsize);
            best = de.get_best();
            if (best < 1e-8)
                solution_found = true;
            de.replacement();
    ...
    log_data(function_number, dim, popsize, "master_slave_de", best_log, time_log, eval_log); 
